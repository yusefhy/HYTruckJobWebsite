...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const robotsTxt = config.robots?.index === false
    ? 'User-agent: *\nDisallow: /\n'
    : 'User-agent: *\nAllow: /\n\nSitemap: https://www.hytruckjob.com/sitemap.xml\n'
...
/**
 * Serves sitemap.xml and robots.txt as raw static files with correct
 * Content-Type headers so crawlers never receive the SPA shell.
 */
function figmaStaticXml(): Plugin {
  return {
    name: 'figma-static-xml',
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
